...
const BaseInfo = ({ basic = {} as BasicInfo, globalSettings, template }: BaseInfoProps) => {
    ...
    const layoutStyles = {
        left: { container: "flex items-start gap-6", leftContent: "flex items-center gap-6 shrink-0", fields: "flex-1 flex flex-wrap gap-x-8 gap-y-2", nameTitle: "text-left min-w-0" },
        right: { container: "flex items-start gap-6 flex-row-reverse", leftContent: "flex justify-end items-center gap-6 shrink-0", fields: "flex-1 flex flex-wrap gap-x-8 gap-y-2", nameTitle: "text-right min-w-0" },
            ...
            <div className={styles.container}>
                ...
                <motion.div layout="position" className={styles.fields} style={{ fontSize: `${globalSettings?.baseFontSize || 14}px`, color: "rgb(75, 85, 99)" }}>
                    {allFields.map((item) => {
                        ...
                        return (
                            <motion.div key={item.key} className="text-baseFont min-w-[200px]">
                                {useIconMode ? (
